...
//array for tracking ingame stats
const gstatArray = {
  "game": 
    {
      "DiePH": [],
...
sessionStorage.setItem("gstatArray",JSON.stringify(gstatArray));  
...
button0.addEventListener("click", getStats)


function getStats() {
  console.log(gstatArray.game.DiePH);
  sessionStorage.clear("gstatArray",JSON.stringify(gstatArray));  
  sessionStorage.setItem("gstatArray",JSON.stringify(gstatArray));  
  window.location.href="../html/ex8.html"
}
...
function playBall() {
  ...
  gstatArray.game.DiePH.push(k);
  gstatArray.game.DieResults.push(i);
  console.log(`die detail: ${i}  die card: ${k} ${gstatArray.game.DiePH.length}`);
